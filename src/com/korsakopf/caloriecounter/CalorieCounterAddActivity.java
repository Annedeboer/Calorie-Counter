		...
		public void onClick(View v) {
			...
			try {
				...
				textNutProt = (EditText) findViewById(R.id.textNutProt);
					...
					if (textNutName.getText().toString().length() != 0){
						
				// Roep de 'insertMultiple' method uit de DbAdapter aan 
				db.insertMultiple(intDate, textNutName.getText().toString(),
						Double.parseDouble(textNutGram.getText().toString()),
						Double.parseDouble(textNutCal.getText().toString()),
						Double.parseDouble(textNutProt.getText().toString()));

				// Toast message die aangeeft wat en hoeveel is toegevoegd
				id = db.getAllEntries();
				Context context = getApplicationContext();
				CharSequence text = "Het etenswaar '" + textNutName.getText()
						+ "' is succesvol toegevoegd\nTotaal aantal rijen in de database is nu: " + id;
				int duration = Toast.LENGTH_LONG;
				Toast toast = Toast.makeText(context, text, duration);
				toast.show();

				// De textvelden weer leegmaken na het toevoegen aan de database
				textNutName.setText("");
				textNutGram.setText("");
				textNutCal.setText("");
				textNutProt.setText("");
						...
						Context context = getApplicationContext();
						CharSequence melding = "test toast";
						int duur = Toast.LENGTH_LONG;
						Toast toast2 = Toast.makeText(context, melding, duur);
						toast2.show();
					}
			} catch (Exception ex) {
			}
